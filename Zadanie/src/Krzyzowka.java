...
public class Krzyzowka {
	...
	// tablica na chromosomy po krzyzowce i mutacji do kolejnych obliczen
		public int[] chrPoKrzyzowce2 = new int[6];
	...
	public void mutuj(){
		...
		if(prawdMut[i]<0.3){
			locus = (int) obiektZadanie.losujPrawd(4);
			 ...
			 j = i+1;
			System.out.println("\nChromosom: "+j+" Mutacja nie zaszla");}
		 
	}
		System.out.print("\nChromosomy po mutacji ");
		for(int i=0; i<6; i++){
			System.out.print(" "+chrPoKrzyzowce[i]);
			chrPoKrzyzowce2[i] = Integer.valueOf(chrPoKrzyzowce[i]);
		}
	...
	public static void main(String[] args) {
		...
		ustawiacz1.ustaw(obiektZadanie.chromosomy);
	...
	//	petla do ustawienia ilosci pokolen
		int iterator=0;
		while(iterator<1000){
			ustawiacz1.ustaw(k1.chrPoKrzyzowce2);

			 maxPrzedzial = obiektZadanie.chrPrzedzialy[5] + 1;
			obiektZadanie.losuj(maxPrzedzial, k1.chrExperyment);
